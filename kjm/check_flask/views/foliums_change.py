...
Original file is located at
    https://colab.research.google.com/drive/18LQkPFtY0CXd7XhH2OO0OF1vtNTuMiAR
"""
...
aaa = MapManager('/content/drive/MyDrive/Colab Notebooks/report_2023_08_12.csv')
aaa

df_report = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/report_2023_08_12.csv')
df_report
...
locations = [[37.3953946671528, 127.1093297131001],
             [37.39417823717656, 127.1093844115068],
             [37.39271861487178, 127.10934841208203],
             [37.39185066864326, 127.11250910446297],
             [37.39345442116072, 127.11257925899302],
             [37.395544745716585, 127.11262755830957],
             [37.39650119524654, 127.111183435904],
             [37.39649911490269, 127.11337434990375],
             [37.39601079572209, 127.11520313115568],
             [37.39440567382231, 127.11653325709659],
             [37.39346684071838, 127.11831608687358],
             [37.39080112282156, 127.11700196362115],
             [37.39188034187261, 127.119013740367],
             [37.3940362949143, 127.10695620640082],
             [37.396431672499325, 127.10842774919402],
             [37.397943736734206, 127.11021431692127]]

df_report['Location'] = locations

# origin_img를 저장하고, 경로 추출
def load_origin_img_path(df, idx):
    origin_img = df.loc[idx,'Origin_img'][0]
    origin_img = np.array(origin_img,dtype=np.uint8)
    img = Image.fromarray(origin_img)
...
# detect_img를 저장하고, 경로 추출
def load_detect_img_path(df, idx):
    detect_img = df.loc[idx,'Detect_img']
    detect_img = np.array(detect_img,dtype=np.uint8)
    img = Image.fromarray(detect_img)
...
# crop된 이미지들을 저장하고, 경로 추출
def load_crop_imgs_path(df, idx):
    crop_imgs = df.loc[idx]['Crop_imgs']
    n_crops = len(crop_imgs)
    crop_path_list = []
...
# 모든 이미지 경로를 저장하고, 경로 추출
def load_imgs_path(df, idx):
    base_path_list = []
    crop_path_list = []

    origin_img_path = load_origin_img_path(df, idx)
    detect_img_path = load_detect_img_path(df, idx)
    base_path_list.append(origin_img_path)
    base_path_list.append(detect_img_path)

    try:
        crop_path_list = load_crop_imgs_path(df,idx)
    except:
        pass
...
def get_crop_htmls(df, idx, crop_path_list, pic_crops):
    htmls = []
    for i in range(len(crop_path_list)):
        if df.loc[idx]['Crop_classes'][i] == 'frame':
            category = df.loc[idx]['Category'][i]
            categories = {-1:'초기화', 0:'프레임', 1:'합법(공익)', 2:'정치', 3:'기타'}
            texts = df.loc[idx]['ClovaOCR_text'][i]
            basis = df.loc[idx]['Category_basis'][i]
...
def get_folium_popups(df, idx):
    ...
    base_h = len(pic_base) * 100
    crop_h = len(df.loc[idx]['Crop_classes']) * 100  # 만약 banner나 frame을 detect하지 못했다면 popup창의 크기를 조절
...
for i in tqdm(range(len(df_report))):
    popup = get_folium_popups(df_report, i)
    location = df_report.iloc[i]['Location']
    categories = df_report.iloc[i]['Category']
