...
class MapManager:
    # report_file_path -> df_report 로 변경
    def __init__(self, report_file_path):
        self.df = report_file_path
...
# df = pd.read_csv('reports/report_2023_08_12.csv')
    ...
    def produce_folder(self):
        ...
        center = eval(self.df.iloc[0]['Location'])  # 지도의 중심좌표
        self.map = folium.Map(location = center, control_scale=True, zoom_start=12)

        # 각 마커별 그룹 생성
        political_group = folium.FeatureGroup(name="정치").add_to(self.map)
        illegal_group = folium.FeatureGroup(name="불법").add_to(self.map)
        legal_group = folium.FeatureGroup(name="합법").add_to(self.map)

        # 마커별 on/off가 가능하게 해주는 코드
        folium.LayerControl(collapsed=False).add_to(self.map)

        for i in tqdm(range(len(self.df))):
            popup = self.get_folium_popups(self.df, i)
            location = self.df.iloc[i]['Location']
            categories = self.df.iloc[i]['Category']

            # 2: 정치, 3: 불법, 그외: 합법
            if 2 in categories:
                marker = folium.Marker(location=location, icon=folium.Icon(color='blue', icon='bookmark'), popup=popup)
                political_group.add_child(marker)
            elif 3 in categories:
                marker = folium.Marker(location=location, icon=folium.Icon(color='red', icon='star'), popup=popup)
                illegal_group.add_child(marker)
            else:
                marker = folium.Marker(location=location, icon=folium.Icon(color='green', icon='check'), popup=popup)
                legal_group.add_child(marker)
        return self.map
